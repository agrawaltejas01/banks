...
type WalletRepo interface {
	...
	UpdateWalletBalance(id string, newFunds int) error
}

type WalletService interface {
	CreateWallet(userId string) (*wallet_model.Wallet, error)
	GetWalletById(id string) (*wallet_model.Wallet, error)
	UpdateWalletBalance(id string, amount int, tType transaction_model.TransactionType) (*wallet_model.Wallet, error)
}
